  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 px-4 dark:from-slate-950 dark:to-slate-900">
      <div className="w-full max-w-md animate-fade-in">
        ...
        >
          <h2 className="mb-4 text-lg font-medium text-slate-900 dark:text-white">Create account</h2>
          {error && (
            <p className="mb-3 rounded-lg bg-red-50 p-2 text-sm text-red-600 dark:bg-red-950 dark:text-red-400">
              {error}
            </p>
          )}
          <label className="mb-1 block text-sm text-slate-600 dark:text-slate-400">Full name</label>
          <input
            type="text"
            required
            value={fullName}
        ...
        <p className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
          Already registered?{' '}
          <Link to="/login" className="font-medium text-slate-600 hover:underline dark:text-slate-200">
            Sign in
          </Link>
        </p>
      </div>
